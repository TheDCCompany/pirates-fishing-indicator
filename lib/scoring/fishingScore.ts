...
type ScoreInput = {
  ...
  cloudCover: number;
  temperature: number;
  sunrise: string; // ISO or "YYYY-MM-DDTHH:MM"
  waveHeight?: number;
  moonPhase: string;
};
...
// Favorable offshore / cross-shore directions for Galveston surf
...
export function computeFishingScore(input: ScoreInput): ScoreResult {
  ...
  if (windSpeed <= 8) {
    reasons.push(`Light wind (${windSpeed} mph) — ideal casting and bait presentation.`);
  } else if (windSpeed <= 14) {
    reasons.push(`Moderate wind (${windSpeed} mph) — manageable for surf fishing.`);
  } else if (windSpeed <= 20) {
    reasons.push(`Elevated wind (${windSpeed} mph) — expect chop and harder casting.`);
  } else {
    reasons.push(`Strong wind (${windSpeed} mph) — rough conditions on the beach.`);
  }
  ...
  // Check if we're in the morning window (sunrise ±2 hrs)
  let inMorningWindow = false;
  if (input.sunrise) {
    const sr = new Date(input.sunrise);
    const windowStart = new Date(sr.getTime() - 30 * 60000);
    const windowEnd = new Date(sr.getTime() + 2 * 60 * 60000);
    inMorningWindow = now >= windowStart && now <= windowEnd;
  }
  ...
  if (input.sunrise) {
    const sr = new Date(input.sunrise);
    const minsFromSunrise = (now.getTime() - sr.getTime()) / 60000;
    if (minsFromSunrise >= -30 && minsFromSunrise <= 90) {
      sunMoon = 13;
      sunMoonReason = "Golden hour — fish are most active in the first 90 minutes after sunrise.";
    } else if (minsFromSunrise > 90 && minsFromSunrise <= 180) {
      sunMoon = 8;
      sunMoonReason = "Morning window — still a good time to fish before midday sun.";
    } else if (minsFromSunrise < -30) {
      sunMoon = 10;
      sunMoonReason = "Pre-sunrise — fish are feeding in low-light conditions.";
    } else {
      sunMoon = 5;
    }
  } else {
  ...
  // --- Surf (15 pts) ---
  let surf = 0;
  let surfLabel: SurfLabel = "fishable";
  ...
  const label: FishingLabel =
    total >= 75 ? "Good" : total >= 50 ? "Fair" : "Poor";
